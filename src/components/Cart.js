                    ...
                    {items.map((cval) => {
                        return (
                            <tr>
                                <td>{cval.id}</td>
                                <td>
                                    <img src={cval.cover} height={"100"} alt="product-img" />
                                </td>
                                <td>{cval.name}</td>
                                <td>{cval.price}</td>
                                <td>{cval.color}</td>
                                <td> <button onClick={() => updateItemQuantity(cval.id, cval.quantity + 1)}>+</button>
                                </td>
                                <td>{cval.quantity}</td>
                                <td>
                                    <button onClick={() => updateItemQuantity(cval.id, cval.quantity - 1)}>-</button></td>
                                <td><button className="btn btn-danger text-dark" onClick={() => removeItem(cval.id)} >Remove</button>
                                </td>
                                {/* <td> <button className='btn btn-danger text-dark'>Remove</button></td>/ */}
                            </tr>
                        );
                    })}
